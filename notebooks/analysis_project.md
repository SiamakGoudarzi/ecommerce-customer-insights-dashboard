...
# 3. Eigene Farben definieren
custom_colors = {
    'Gelegenheitskäufer (Cluster 0)': '#1f77b4',   # Blau
    'Inaktive Kunden (Cluster 1)': '#ff7f0e',      # Orange
    'Champions / Top-Kunden (Cluster 2)': '#2ca02c' # Grün
}
...
1. **Cluster 2 (Grün) – Die "Champions / Top-Kunden":**
...
3. **Cluster 1 (Orange) – Die "Inaktiven / Abgewanderten Kunden":**
...
## Ziel dieses Schritts
Die mathematischen Ergebnisse des Clusterings werden in umsetzbare Marketing-Strategien überführt. Dabei liegt der Fokus auf der Maximierung des Customer Lifetime Value (CLV) bei gleichzeitiger Ressourcen-Effizienz.

---

## 1. Champions (Cluster 2 - Grün)
* **Charakteristik:** Höchste Umsatzanteile, hohe Kauffrequenz.
* **Hypothese:** Aufgrund des hohen Warenkorbvolumens könnte es sich teilweise um B2B-Kunden oder professionelle Wiederverkäufer handeln. 
    * *Hinweis: Eine Verifizierung über Produktkategorien (z. B. Bulk-Produkte) ist zur Validierung notwendig.*
* **Strategische Maßnahmen (Retention & VIP):**
    * **Exklusive VIP-Programme:** Einführung eines persönlichen Account-Managements für Top-Performer.
    * **Priorisierter Service:** Bevorzugte Abwicklung bei Engpässen und exklusive Frühzugänge zu neuen Sortimenten.
    * **Content-Strategie:** Gezielte Ansprache durch exklusive Newsletter mit Fokus auf Sortimentserweiterungen statt simpler Rabatte.

## 2. Gelegenheitskäufer (Cluster 0 - Blau)
* **Charakteristik:** Solides Mittelfeld, Potenzial für Upselling.
* **Strategische Maßnahmen (Upselling & Cross-Selling):**
    * **Warenkorb-Optimierung:** Einsatz von Recommendation-Engines (z. B. „Häufig zusammen gekauft“), um den durchschnittlichen Bestellwert (AOV) gezielt zu steigern.
    * **Incentivierung:** Einführung von "Stufen-Gutscheinen" (z. B. 10€ Rabatt ab 100€ Bestellwert), um Anreize für größere Einkäufe zu schaffen.
    * **SEO-Strategie:** Fokus auf transaktionale Keywords (vorbehaltlich einer tieferen Keyword-Recherche).

## 3. Inaktive Kunden (Cluster 1 - Orange)
* **Charakteristik:** Längere Inaktivität, geringe Kauffrequenz.
* **Strategische Maßnahmen (Reaktivierung & Kostenkontrolle):**
    * **Win-Back-Zyklus:** 
        * **Phase 1 (Tag 0):** Automatisierte Reaktivierungs-Mail mit einem attraktiven "We miss you"-Angebot.
        * **Phase 2 (Tag 30):** Zweiter Kontaktpunkt (z. B. Reminder oder Kundenbefragung „Warum haben Sie uns verlassen?“).
        * **Phase 3 (ab Tag 90):** Bei Erfolglosigkeit wird das Segment in die Kategorie "Low-Priority" verschoben, um das Marketingbudget primär auf Cluster 0 und 2 zu fokussieren.
    * **Content-Strategie:** Fokus auf informative, hilfreiche Long-Tail-Inhalte, die organisch (ohne bezahlte Ads) Aufmerksamkeit generieren.
...
# Schritt 9: Prädiktive Churn-Modellierung

## Der Übergang von Deskription zu Prädiktion
Der Übergang vom unüberwachten Clustering zur überwachten Klassifikation ist der konzeptuell wichtigste Schritt. Während das Clustering (Schritt 6) Gruppen basierend auf Ähnlichkeiten fand, definieren wir nun ein binäres `Churn`-Label, um abwanderndes Verhalten vorherzusagen.

## Methodik & Implementierung
1. **Definition der Zielvariable:** Kunden aus dem inaktiven Cluster (Cluster 1) werden als `Churn = 1` markiert. 
   * *Methodischer Hinweis:* Da das Churn-Label aus dem Clustering (RFM-basiert) abgeleitet wurde und die Modell-Features ebenfalls auf RFM basieren, liegt konzeptuell ein hoher Informationsüberlapp vor. In einem produktiven System würde man dies durch einen zeitlichen Split (Training auf Vergangenheit, Test auf Zukunft) lösen. Als Lernprojekt ist die gewählte Methode zur Validierung der RFM-Logik jedoch vollständig valide.

2. **Algorithmus (Random Forest):** 
   - Wir nutzen ein Ensemble aus 100 Entscheidungsbäumen.
   - **Vorteile:** Robustheit gegenüber Ausreißern, keine Notwendigkeit zur Feature-Skalierung, und direkte Interpretierbarkeit durch `feature_importances_`.
   - **Regularisierung:** Durch `max_depth=6` wird das Modell vor Overfitting geschützt, um die Generalisierung auf neue Kundendaten zu gewährleisten.

3. **Datenaufteilung:**
   - Wir verwenden einen 80/20-Split. Der Einsatz von `stratify=y` ist eine kluge Vorsichtsmaßnahme, um das Klassenverhältnis (Churn vs. Aktiv) in beiden Datensätzen beizubehalten und eine verzerrungsfreie Evaluierung sicherzustellen.
...
# Schritt 10: Interpretation der Modell-Evaluierung

Die empirische Evaluation des Random Forest Classifiers auf dem Testdatensatz (1.176 Kunden) liefert exzellente mathematische Metriken. Es ist jedoch essenziell, diese Ergebnisse im Kontext der methodischen Herleitung zu interpretieren.

## 1. Leistung des Modells (Metriken)
* **Accuracy (98,9%):** Das Modell klassifiziert fast jede Kundeninstanz korrekt.
* **Precision & Recall:** Beide Werte liegen nahezu bei 100%. Die Confusion Matrix bestätigt, dass Fehlalarme (False Positives: 5) sowie übersehene Abwanderer (False Negatives: 7) vernachlässigbar gering sind.

## 2. Kritische Einordnung (Reflexion der Label-Genese)
Die fast perfekten Metriken sind kein Zufall, sondern methodisch erklärbar:
* **Informationsüberlapp:** Das `Churn`-Label basiert auf `Cluster 1`, welches wiederum aus den RFM-Features abgeleitet wurde. Da unser Modell dieselben RFM-Features als Input nutzt, hat der Random Forest im Kern die Entscheidungsgrenzen des K-Means-Clustering gelernt. 
* **Kein "Label Leakage" im negativen Sinne, sondern eine hochpräzise Rekonstruktion:** Das Modell fungiert somit als extrem effizienter Proxy für das Clustering. Dies ist praktisch sehr nützlich, da neue Kunden nun ohne erneute K-Means-Berechnung sofort klassifiziert werden können.

## 3. Zusammenfassung & Ausblick
* **Praktischer Nutzen:** Als *Proof-of-Concept* für die Pipeline ist das Modell exzellent geeignet. Wir haben bewiesen, dass ein Random Forest komplexe Segmentierungskriterien in einfache Entscheidungsregeln übersetzen kann.
* **Produktionsreife:** Für eine echte *Churn-Prognose* (Vorhersage zukünftigen Verhaltens) wäre in einem nächsten Schritt eine zeitliche Trennung erforderlich (Training auf Daten von Jahr A, Evaluation auf Kaufaktivität in Jahr B). Dennoch bietet das aktuelle Modell eine robuste Basis für eine automatisierte CRM-Segmentierung basierend auf der bestehenden RFM-Struktur.
